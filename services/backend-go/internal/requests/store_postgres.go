...
func (s *PostgresRequestStore) Create(ctx context.Context, request DealRequest) error {
	q := `INSERT INTO deal_requests (
		...
		message, requester_wallet, consent_accepted, status
	) VALUES (
	...
	_, err := s.pool.Exec(ctx, q,
		...
		request.AmountRange, request.Chain, request.Message, request.RequesterWallet, request.ConsentAccepted, request.Status,
	)
	return err
}
