...
type Config struct {
	...
	GnoChainID                 string
	GnoRPCURL                  string
	GnoEscrowRealmPath         string
	GnoSettlementEnabled       bool
	GnoDeploymentMode          string
}
...
func Init() {
	AppConfig = &Config{
		...
		WSRateLimitPerMinute:       getEnvInt("WS_RATE_LIMIT_PER_MINUTE", 120),
		CountdownSeconds:           getEnvInt("COUNTDOWN_SECONDS", 10),
		IntentTTLMinutes:           getEnvInt("INTENT_TTL_MINUTES", 15),
		...
		GnoChainID:                 getEnvString("GNO_CHAIN_ID", "gno-testnet"),
		GnoRPCURL:                  getEnvString("GNO_RPC_URL", ""),
		GnoEscrowRealmPath:         getEnvString("GNO_ESCROW_REALM_PATH", "gno.land/r/tradewindow/escrow"),
		GnoSettlementEnabled:       getEnvBool("GNO_SETTLEMENT_ENABLED", false),
		GnoDeploymentMode:          getEnvString("GNO_DEPLOYMENT_MODE", "local"),
	}
}
...
func getEnvBool(key string, fallback bool) bool {
	if value, exists := os.LookupEnv(key); exists {
		...
		if err != nil {
			log.Printf("Warning: Invalid boolean value for %s: %v. Using default: %t\n", key, value, fallback)
			return fallback
		}
		return parsed
	}
