...
import { getTradeClientId } from '../lib/websocket/client-id';
...
export function useTradeRoom(walletAddress: string) {
  ...
  useEffect(() => {
    ...
    const clientId = getTradeClientId();
    const ws = new WebSocket(
      `${wsUrl}?wallet=${encodeURIComponent(walletAddress)}&clientId=${encodeURIComponent(clientId)}`
    );
    wsRef.current = ws;
