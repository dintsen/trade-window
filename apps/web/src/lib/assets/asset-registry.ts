...
export function getAssetByChainAndDenom(
  chainId: string,
  technicalDenom: string
): AssetIdentity | undefined {
  const normalized = normalizeDenom(technicalDenom);
  return getAllAssets().find(
    (asset) => asset.chainId === chainId && asset.technicalDenom === normalized
  );
}

export function getAssetsByDisplayDenom(displayDenom: string): AssetIdentity[] {
  const normalized = normalizeDenom(displayDenom);
  return getAllAssets().filter(
    (asset) =>
      asset.symbol.toLowerCase() === normalized ||
      asset.displayDenom.toLowerCase() === normalized
  );
}
