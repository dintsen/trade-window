...
import { Database, Cpu, ImageIcon, FileSignature, CheckCircle2, ChevronRight, HelpCircle, ArrowUpRight, ShieldCheck } from 'lucide-react';
...
func CreateCommitment(id, intentHash, partyA, partyB string) string {
    ...
    {
      title: '4. Atomic Escrow Settlement',
      desc: 'The Escrow contract releases deposits. In the current MVP, transfers are routed as direct bank sends via wallet connections.',
      status: 'pending',
      contract: 'gno/realms/tradewindow/escrow (future)',
      codeSnippet: `// Future Escrow settlement design
func SettleTrade(id string) {
    if !intents.IsDualSigned(id) { panic("not dual signed") }
    // std.GetBanker().SendCoins(...)
    // releases locks and swaps assets atomically
}`
    }
  ];
                ...
                <div className="p-4 border-t border-white/5 bg-[#0b0b0f] flex items-center gap-3 text-xs text-white/40 leading-normal">
                  ...
                  <div>
                    {steps[activeStep].contract ? (
                      <span>
                        Committed on-chain. Invoked securely from the user&apos;s browser using a connected wallet (such as Adena) via contract transactions.
                      </span>
                    ) : (
                      <span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-white/45 leading-relaxed">
                        Before listing, assets are audited against our Gno.land <code className="text-pink-400 font-mono text-xs">registry.gno</code> realm. Suspicious or easily-faked tokens are flagged instantly to alert the receiver of potential scams.
                      </p>
                    </div>
                  </div>
                </div>
