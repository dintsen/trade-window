...
| 7 | **Intent `Nonce` was room ID** — not random, replayable across sessions | Medium | `randomNonce()` generates 16 random bytes per intent |
| 8 | **Intent `CreatedAt`/`ExpiresAt` were empty strings** | Low | Set to current time and `+10min` in RFC3339 |
...
|---|--------------|----------|-----|
| G1 | **`intents.gno:CreateCommitment`** — no caller check. Anyone could create a commitment with arbitrary partyA/partyB | **CRITICAL** | Added `std.GetOrigCaller()` check: caller must be partyA or partyB |
...
The Gno.land realms are **on-chain intent scaffolds only**. They do NOT:
- Hold escrow (no `std.Send` calls)
- Atomically swap assets between parties
- Enforce settlement on-chain

The intents realm records that both parties signed an agreed hash. **Actual asset transfer is still a manual step.** This is by design for MVP. Cross-chain atomic settlement is out of scope per `PROJECT_CONTEXT.md`.
