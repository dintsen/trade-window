...
Trade Window supports Stargaze NFT discovery for connected Cosmos wallets (Keplr, Leap, Cosmostation). Users can view their Stargaze NFT holdings and select an NFT as the asset in an OTC listing or deal request.

NFT **transfers are not executed**. The platform records the NFT metadata as the trade intent (chain, collection, token ID, image). Settlement remains out of scope for MVP.
...
NFT cards are rendered in a grid on `/board/new` under a **"NFT Assets (Preview)"** tab when a Cosmos wallet is connected.
